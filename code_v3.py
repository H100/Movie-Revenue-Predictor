...
"""
# Plot outputs
plt.scatter(Y_test_rate, y_lin,  color='black')
plt.plot(Y_test_rate, y_lin, color='blue',linewidth=3)
...
encoded = lab_enc.fit_transform(Y_train_rate)
encoded_test = lab_enc.fit_transform(Y_test_rate)

clf = svm.SVC(kernel='rbf', gamma=0.7, C = 1.0).fit(X_train_rate, encoded)
y_predicted = clf.predict(X_test_rate)

# performance
print("Classification report")

print(metrics.classification_report(encoded_test, y_predicted))

print("Confusion matrix")
print(metrics.confusion_matrix(encoded_test, y_predicted))
